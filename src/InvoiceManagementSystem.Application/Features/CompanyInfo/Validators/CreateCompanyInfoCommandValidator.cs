...
namespace InvoiceManagementSystem.Application.Features.CompanyInfo.Validators
{
    public class CreateCompanyInfoCommandValidator : AbstractValidator<CreateCompanyInfoCommand>
    {
        ...
        public CreateCompanyInfoCommandValidator(IApplicationDbContext context)
        {
            _context = context;
            ...
            RuleFor(x => x.Address)
                ...
                .MaximumLength(50).WithMessage("Address must not exceed 255 characters.");
            RuleFor(x => x.ZipCode)
                .NotEmpty().WithMessage("Zip code is required.")
                .MaximumLength(20).WithMessage("Zip code must not exceed 255 characters.");
            RuleFor(x => x.City)
                .NotEmpty().WithMessage("City is required.")
                .MaximumLength(50).WithMessage("city must not exceed 255 characters.");
            RuleFor(x => x.Country)
                .NotEmpty().WithMessage("Country is required.")
                .MaximumLength(50).WithMessage("Country must not exceed 255 characters.");
        }
